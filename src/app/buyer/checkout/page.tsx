...
export default function CheckoutPage() {
  ...
  if (!items.length) return (
    <div className="max-w-[860px] mx-auto px-[14px] py-12 text-center">
      <p className="text-5xl mb-4">🛒</p><p className="font-[800] text-lg mb-2">Cart is empty</p>
        ...
        <div className="grid md:grid-cols-[1fr_340px] gap-4 items-start">
          <div className="space-y-4">
            <div className="bg-white rounded-[6px] p-[18px] shadow-[var(--sh)]">
              <h2 className="text-[16px] font-[800] mb-4 pb-[10px] border-b border-[#E8E8E8]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>Delivery Details</h2>
              ...
              <div><label className="block text-[11px] font-[800] text-[#757575] uppercase tracking-[.06em] mb-[5px]">Note (optional)</label><input value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} placeholder="Any special instructions?" className="w-full px-[14px] py-[11px] border-[1.5px] border-[#D0D0D0] rounded-[10px] text-sm outline-none focus:border-[#F85606] transition-colors" /></div>
            </div>
            <div className="bg-white rounded-[6px] p-[18px] shadow-[var(--sh)]">
              <h2 className="text-[16px] font-[800] mb-3 pb-[10px] border-b border-[#E8E8E8]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>Payment</h2>
              ...
              <p className="text-[11px] text-[#757575] text-center mt-2">Powered by Paystack · SSL Encrypted</p>
            </div>
          </div>
          <div className="bg-white rounded-[6px] p-[18px] shadow-[var(--sh)] sticky top-[108px]">
            <h2 className="text-[16px] font-[800] mb-4 pb-[10px] border-b border-[#E8E8E8]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>Order Summary</h2>
            {items.map((item) => {
              const img = parseImages(item.images)[0]
              return (
                <div key={item.product_uuid} className="flex gap-[10px] py-[9px] border-b border-[#E8E8E8]">
                  <div className="w-12 h-12 rounded-[6px] flex-shrink-0 overflow-hidden bg-[#F5F5F5] flex items-center justify-center">{img ? <img src={img} alt={item.name} className="w-full h-full object-cover" /> : <span className="text-xl">📦</span>}</div>
            ...
            <div className="mt-3 space-y-1">
              ...
              <div className="flex justify-between text-[13px]"><span>Delivery</span><span className="text-[#00853D] font-[700]">Free</span></div>
              <div className="flex justify-between text-[16px] font-[800] border-t-2 border-[#E8E8E8] pt-2 mt-2">
                <span>Total</span><span className="text-[#F85606] text-[19px]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>{ngn(total)}</span>
              </div>
            </div>
          </div>
        </div>
