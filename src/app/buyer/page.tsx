...
const CATEGORIES = [
  { slug: 'smartphones', label: 'Phones', icon: '📱' }, { slug: 'laptops', label: 'Laptops', icon: '💻' },
  { slug: 'womens-clothing', label: "Women's", icon: '👗' }, { slug: 'mens-clothing', label: "Men's", icon: '👔' },
  { slug: 'beauty', label: 'Beauty', icon: '✨' }, { slug: 'groceries', label: 'Groceries', icon: '🛒' },
  { slug: 'home-decoration', label: 'Home', icon: '🛋️' }, { slug: 'sports-accessories', label: 'Sports', icon: '🏋️' },
  { slug: 'fragrances', label: 'Fragrances', icon: '🌸' }, { slug: 'skin-care', label: 'Skincare', icon: '💧' },
]
  ...
  return (
    <div className="page-enter">
      ...
      <div className="relative overflow-hidden bg-[#1a1a1a] mb-[10px]" style={{ height: 300 }}>
        <div className="flex h-full transition-transform duration-500 ease-[cubic-bezier(.4,0,.2,1)]" style={{ transform: `translateX(-${heroIdx * 100}%)`, width: `${SLIDES.length * 100}%` }}>
          {SLIDES.map((s, i) => (
            <div key={i} className="relative flex items-center overflow-hidden flex-shrink-0" style={{ width: `${100 / SLIDES.length}%`, background: s.bg }}>
              ...
              <div className="relative z-10 px-11 max-w-[540px]">
                ...
                <h1 className="text-white font-[800] leading-[1.15] mb-2 text-[clamp(18px,3vw,32px)]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>{s.title}</h1>
        ...
        <div className="absolute bottom-[10px] left-1/2 -translate-x-1/2 flex gap-[6px] z-20">
          {SLIDES.map((_, i) => (
            <button key={i} onClick={() => goSlide(i)} className={`h-[7px] rounded-full transition-all duration-300 ${i === heroIdx ? 'w-5 bg-white' : 'w-[7px] bg-white/50'}`} />
          ))}
        </div>
        <button onClick={() => goSlide((heroIdx - 1 + SLIDES.length) % SLIDES.length)} className="absolute left-[10px] top-1/2 -translate-y-1/2 w-[34px] h-[34px] bg-white/80 rounded-full flex items-center justify-center hover:bg-white z-20">‹</button>
        <button onClick={() => goSlide((heroIdx + 1) % SLIDES.length)} className="absolute right-[10px] top-1/2 -translate-y-1/2 w-[34px] h-[34px] bg-white/80 rounded-full flex items-center justify-center hover:bg-white z-20">›</button>
      </div>
      ...
      <div className="bg-white mb-[10px]">
        <div className="bg-gradient-to-r from-[#E01D1D] to-[#FF4444] px-4 py-[10px] flex items-center gap-3 flex-wrap">
          <span className="text-white font-[800] text-base" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>⚡ Flash Sale</span>
      ...
      {vendors.length > 0 && (
        <div className="bg-gradient-to-r from-[#FFF8E6] to-[#FEF0CC] border-y-2 border-[#FFC200] mb-[10px] py-[14px]">
          <div className="flex items-center justify-between px-4 mb-3">
            <h2 className="font-[800] text-[15px] flex items-center gap-2" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>⭐ Spotlight Vendors</h2>
            ...
            {vendors.slice(0, 8).map((v) => (
              <div key={v.uuid} onClick={() => router.push(`/buyer/vendor/${v.uuid}`)}
                ...
                <div className="h-[80px] flex items-center justify-center bg-gradient-to-br from-[#FFF3E0] to-[#FFE0B2] relative">
                  {v.profile_photo ? <img src={v.profile_photo} alt={v.name} className="w-12 h-12 rounded-full object-cover" /> : <span className="text-3xl">🏪</span>}
                  <span className="absolute top-[6px] left-[6px] bg-[#FFC200] text-black text-[9px] font-[900] px-2 py-[2px] rounded-full">★ Spotlight</span>
                </div>
      ...
      <div className="bg-white mb-[10px] px-4 py-[14px]">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-[800] text-[15px]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>Shop by Category</h2>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(72px,1fr))] gap-[10px]">
          {CATEGORIES.map((c) => (
            <button key={c.slug} onClick={() => router.push(`/buyer/category/${c.slug}`)}
              ...
              <div className="w-[54px] h-[54px] rounded-[14px] bg-[#F5F5F5] flex items-center justify-center text-2xl group-hover:bg-[#FFF3EE] transition-colors">{c.icon}</div>
      ...
      <div className="grid grid-cols-2 gap-[10px] px-[14px] mb-[10px]">
        <button onClick={() => router.push('/buyer/category/smartphones')}
          ...
          <span className="text-[10px] font-[900] text-[#FFC200] uppercase tracking-[.06em] mb-1">Trending</span>
          <span className="text-white font-[800] text-[16px] leading-[1.2] block mb-2" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>Top Phones & Gadgets</span>
        ...
        <button onClick={() => router.push('/buyer/category/beauty')}
          ...
          <span className="text-[10px] font-[900] text-[#e9d5ff] uppercase tracking-[.06em] mb-1">Beauty Deals</span>
          <span className="text-white font-[800] text-[16px] leading-[1.2] block mb-2" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>Up to 40% Off</span>
      ...
      {SECTIONS.map(({ label, cat }) => (
        <div key={cat} className="bg-white mb-[10px]">
          <div className="flex items-center justify-between px-4 pt-3">
            <h2 className="font-[800] text-[15px]" style={{ fontFamily: 'var(--font-syne, system-ui)' }}>{label}</h2>
            <button onClick={() => router.push(`/buyer/category/${cat}`)}
              className="bg-[#F85606] text-white text-[11px] font-[800] px-[14px] py-[5px] rounded-full hover:bg-[#e84e05] transition-colors">See all</button>
          </div>
